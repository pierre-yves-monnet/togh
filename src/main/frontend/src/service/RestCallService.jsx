...
debugger;
        ...
        console.log("RestCallService.determineUrlHost urlPing["+urlPing+"]");
        ...
        // Now react is on the same server as the back office
        if (window.location.host ==="http://localhost:3000") {
            self.serverHttp= "http://localhost:7080";
            console.log("RestCallService.determineUrlHost serverHttp return ["+self.serverHttp+"] but override with http://localhost:7080");
        } else {
            console.log("RestCallService.determineUrlHost serverHttp return ["+self.serverHttp+"] but override with [] (rect on same server)");
          self.serverHttp="";
        }
